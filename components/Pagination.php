...
class Pagination
{
    ...
    /**
     * @var Текст ссылки на первую страницу
     */
    const TEXT_TO_BEGINNING = 'В начало';

    /**
     * @var Текст ссылки на последнюю страницу
     */
    const TEXT_TO_END = 'В конец';
    ...
    public function get()
    {
        ...
        $html = '<nav aria-label="..."><ul class="pagination justify-content-end">';
        # Генерируем ссылки
        for ($page = $limits[0]; $page <= $limits[1]; $page++) {
            # Если текущая это текущая страница, ссылки нет и добавляется класс active
            if ($page == $this->current_page) {
                $links .= '<li class="page-item active" aria-current="page"><span class="page-link">' . $page . '<span class="sr-only">(current)</span></span></li>';
            } else {
        ...
        if (!is_null($links)) {
            ...
            if ($this->current_page > 1) {
                ...
                $links = $this->generateHtml(1, self::TEXT_TO_BEGINNING) . $links;
            }

            # Если текущая страница не первая
            if ($this->current_page < $this->amount) {
                # Создаём ссылку "На последнюю"
                $links .= $this->generateHtml($this->amount, self::TEXT_TO_END);
            }
        }
